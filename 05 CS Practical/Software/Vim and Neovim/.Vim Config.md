...
Until distinguishing is required, we will need to
...
Customization can be done for specific languages, so any time theyre run, scripts under the language dir will run and configure the files before you see it. Organise it with self-named "profile" folders.

## Random Tips and Tricks

- `init.lua` - initialization file, runs each time a new instance of neovim is created, and can be specified for every folder that config info is drawn from.
...
> Changes are outlined using `remap.lua`.
>
...
For getting packages like fuzy-finders (or a plugin called harpoon). We will be using [`packer.nvim`](https://github.com/wbthomason/packer.nvim)

> [!TIP] installing package managers like <code>packer.lua</code>
> Under nvim > language > profile folder
>
> - `vim.g.mapleader` - variable string of characters vim watches for  command executions
> - `vim.keymap.set(<mode>,<new_bind>,<original_function>)` - while in a specific `mode`, the trigger for `original_function` is now `new_bind`.
>     - your bindings can have the `<header>` prefix to indicate they follow the header.
...
- `Ctrl+p` &#8594; `<space>pv` - searches through git file history
    - note in the code itself, `Ctrl+P` follows syntax `<C-p>`.
    - syntax is `<<press_and_hold_key_short>-<key>>`
        - ie declaring `<a-p>` indicates pressing and holding Alt then p.
- `<space>ps` - searches through all files for a specific user-entered string
...
CAA 28 Mar 2025, Neovim now has its own LSP doesn't need new plugins. [Full Installation guide here](https://lsp-zero.netlify.app/blog/lsp-config-overview.html).https://lsp-zero.netlify.app/blog/lsp-config-without-plugins
...
You can just use [Mason](https://github.com/mason-org/mason.nvim) as a package manager to install stuff
